...
---
48. 什么是对象解构？
也是一种JS表达式，通过把对象（也可以是数组）中的属性取出，也可以说是拆包，赋值给一系列其他变量的操作叫做解构赋值。这是一种比较方便的赋值方式。
...
两种其实都用...的写法来实现，展开运算符是将数组转为按都好分割的有序数列。剩余运算符则更用于解构数组和对象
...
JS的基本类型：String、Number、BigInt、Boolean、Null、undefined、Symbol
...
61. 如何检查对象中是否存在某个属性？
1. "prop" in obj
2. o.hasOwnProperty("prop")
3. o["prop"] 不为undefined则存在
...
1. 创建空对象{}
2. 将空对象分配给this{}
3. 将空对象的proto指向构造函数的prototype
4. 如果没有使用显示return，则返回this
